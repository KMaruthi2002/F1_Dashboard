...
export async function circuitOutline(sessionKey, drivers, revalidate = 21600) {
  ...
  let laps = [];
  try {
    laps = await fetch(`${OPENF1}/laps?session_key=${sessionKey}`, {
      next: { revalidate },
      headers: (() => { const h = { Accept: 'application/json' }; return h; })(),
    }).then((r) => (r.ok ? r.json() : []));
  } catch { laps = []; }
  ...
  const tryDriver = async (num) => {
    ...
    if (cand) {
      ...
      const rows = await fetch(
        `${OPENF1}/location?session_key=${sessionKey}&driver_number=${num}&date>${isoSec(t0 - 800)}&date<${isoSec(t1)}`,
