...
export async function openf1(path, revalidate = 30) {
  // OPENF1_API_KEY (Sponsor tier) unlocks live data during sessions + higher rate limits.
  // Set it in your Vercel/Netlify env vars — never expose it client-side.
  const headers = { Accept: 'application/json' };
  if (process.env.OPENF1_API_KEY) headers.Authorization = `Bearer ${process.env.OPENF1_API_KEY}`;

  // retry on 429 — OpenF1 rate-limits bursts
