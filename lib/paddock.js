...
// ── Pit Wall forum (per-topic message threads in Blob) ──
export const validTopic = (t) => typeof t === 'string' && /^[a-z0-9-]{1,24}$/.test(t);
const forumPath = (topic) => `paddock/forum/${topic}.json`;

export async function getForum(topic) {
  if (!hasBlob() || !validTopic(topic)) return [];
  const { get } = await import('@vercel/blob');
  const res = await get(forumPath(topic), { access: 'private', useCache: false, ...((blobToken()) ? { token: blobToken() } : {}) }).catch(() => null);
...
export async function postForum(topic, message) {
  const { put } = await import('@vercel/blob');
  const messages = await getForum(topic);
  messages.push(message);
  const trimmed = messages.slice(-200); // keep the last 200 per thread
  await put(forumPath(topic), JSON.stringify(trimmed), {
    access: 'private',
    contentType: 'application/json',
    addRandomSuffix: false,
    allowOverwrite: true,
...
export async function getRoundTimes(round) {
  ...
  if (!race) return null;
  // Jolpica gives UTC times with a trailing Z; append one defensively if absent
  // so a missing Z can never shift locks by the server's local offset.
  const t = (s) => {
    if (!s?.date) return null;
    let time = s.time || '00:00:00Z';
    if (!/[zZ]|[+-]\d\d:?\d\d$/.test(time)) time += 'Z';
